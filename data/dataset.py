...
class RefCOCODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ── Processor ─────────────────────────────────────────────────────────
        # 视觉分辨率上限（max_pixels）只在 AutoProcessor.from_pretrained 时生效；
        # 这版 Qwen image processor 会静默忽略调用时再传的 max_pixels，故此处不传。
...
def collate_fn(batch: List[Dict], pad_token_id: int = 0) -> Dict:
    ...
    result = {
        ...
        "gt_boxes": torch.stack([x["gt_boxes"] for x in batch]),
